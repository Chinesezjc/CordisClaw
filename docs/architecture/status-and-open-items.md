...
> 2026-06-05 更新：Web 搜索/抓取 + Git 操作已实现，工具总数 15→21。

当前 agent 有 21 个工具（read/write/search/run/revert/runtime ops/web/git），但仍缺少：
- [x] Web fetch / search — `web_search` (DuckDuckGo), `web_fetch` (HTTP GET, HTML strip)
- [x] Git 操作 — `git_diff`, `git_log`, `git_status`, `git_commit`
