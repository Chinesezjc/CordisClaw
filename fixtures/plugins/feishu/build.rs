...
fn main() {
    let src = "/root/CordisClaw/fixtures/plugins/target/debug/libfeishu.so";
    let dst = "/root/CordisClaw/fixtures/artifacts/feishu.so";
                    ...
                    if let Some(entries) = json_val.get_mut("entries") {
                        if let Some(arr) = entries.as_array_mut() {
                            for entry in arr.iter_mut() {
                                if entry.get("plugin_path").and_then(|v| v.as_str())
                                    == Some("feishu")
                                {
