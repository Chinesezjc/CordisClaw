...
use crate::core::models::{PluginDocs, PluginLoadResult, PluginUnavailableReason};
...
impl PluginRegistry {
    ...
    pub fn mark_unavailable(&mut self, plugin_path: &str, reason: PluginUnavailableReason) {
        if let Some(plugin) = self.plugins.get_mut(plugin_path) {
